            ...
            <div className="flex justify-between items-center">
              <div className="flex items-center space-x-4">
                ...
                <div className="flex items-center space-x-4">
                  ...
                  <div>
                    <h1 className="text-3xl font-bold text-gray-900">Production Planning Department</h1>
                    <p className="text-gray-600 text-base font-medium">Production Order Management System</p>
                  </div>
                </div>
              </div>
                ...
                <form onSubmit={handleCreateOrder} className="space-y-6">
                  ...
                  <div className="relative space-y-2">
                    ...
                    {showDropdown && searchTerm && (
                      <div className="absolute z-10 w-full max-h-48 overflow-y-auto bg-white rounded-lg mt-1 border-2 border-gray-200 shadow-xl">
                        {filteredProducts.length > 0 ? (
                          filteredProducts.map(p => (
                            <div
                              key={p.name}
                              onClick={() => handleItemSelect(p.name)}
                              className={`p-3 cursor-pointer hover:bg-blue-50 text-gray-900 font-medium border-b border-gray-100 last:border-b-0 ${
                    ...
                    {ordersLoading ? (
                      ...
                      orders.map((order, index) => (
                        <motion.div 
                          ...
                          className="bg-gradient-to-r from-gray-50 to-blue-50 border-2 border-gray-200 rounded-lg p-5 hover:shadow-md transition-shadow duration-200"
                        >
                          <div className="flex justify-between items-start">
                            <div className="flex-1">
                              <div className="flex items-center mb-2">
                                <Package className="w-5 h-5 text-blue-600 mr-2" />
                                <h4 className="font-bold text-gray-900 text-lg">{order.productName}</h4>
                              </div>
                              <div className="space-y-1 text-sm">
                                <div className="flex items-center text-gray-700">
                                  <span className="font-medium w-20">Quantity:</span>
                                  <span className="font-bold text-blue-700">{order.quantity} units</span>
                                </div>
                                <div className="flex items-center text-gray-700">
                            ...
                            <div className="ml-4">
                              <Badge className={`${getStatusBadgeColor(getOrderStatus(order.id))} font-bold text-sm px-3 py-2 rounded-full`}>
                                <ShoppingCart className="w-4 h-4 mr-2" />
                                {getOrderStatus(order.id).toUpperCase()}
                              </Badge>
                            </div>
                          </div>
                        </motion.div>
